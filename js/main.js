document.addEventListener('DOMContentLoaded', () => {
    ...
    function runSimulationStep() {
        try {
            ...
            // Simulate page accesses by processes
            const pageAccesses = [];
            const activeProcesses = processes.filter(p => !p.isCompleted);

            if (activeProcesses.length > 0) {
                // Ensure executingProcess gets accesses
                const executingProcess = activeProcesses.find(p => p.id === memoryManager.getCurrentExecutingProcess());
                if (executingProcess) {
                    pageAccesses.push(executingProcess.id, executingProcess.id);
                }

                // Add some random accesses from other active processes
                for (let j = 0; j < 2; j++) {
                    const randomProcess = activeProcesses[Math.floor(Math.random() * activeProcesses.length)];
                    pageAccesses.push(randomProcess.id);
                }
            }
